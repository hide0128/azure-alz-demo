...
# Service Principalを作成（OIDC用）
az ad app create --display-name "github-actions-alz-demo"

# Application IDを取得（AZURE_CLIENT_ID）
APP_ID=$(az ad app list --display-name "github-actions-alz-demo" --query [0].appId -o tsv)
...
GitHub Actionsとの信頼関係を設定します。

```bash
# リポジトリの情報を設定
GITHUB_ORG="your-github-org"
GITHUB_REPO="azure-alz-demo"
...
# Federated Credentialを作成（Pull Request用）
az ad app federated-credential create \
  --id $APP_ID \
  --parameters "{
...
| Secret名 | 説明 | 取得方法 |
|---------|------|---------|
| `AZURE_CLIENT_ID` | Service Principal Application ID | 上記手順で取得した`$APP_ID` |
| `AZURE_TENANT_ID` | Azure Tenant ID | 上記手順で取得した`$TENANT_ID` |
| `AZURE_SUBSCRIPTION_ID` | Azure Subscription ID | 上記手順で取得した`$SUBSCRIPTION_ID` |
| `AZURE_RESOURCE_GROUP` | デプロイ先リソースグループ名 | `rg-landingzone-demo` |
...
gh secret set AZURE_RESOURCE_GROUP --body "rg-landingzone-demo"
...
**エラー**: `Error: Login failed with Error: Unable to get ACTIONS_ID_TOKEN_REQUEST_URL env variable`

**原因**: Federated Credentialsが正しく設定されていない
